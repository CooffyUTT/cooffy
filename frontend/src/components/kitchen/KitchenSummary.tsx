...
export function KitchenSummary({ orders }: KitchenSummaryProps) {
  // Usamos 'state' que es la propiedad real en KitchenOrder
  const pendingCount = orders.filter(o => o.state === 'pending').length;
  const preparingCount = orders.filter(o => o.state === 'preparing').length;
  const readyCount = orders.filter(o => o.state === 'ready').length;
  const pickedUpCount = orders.filter(o => o.state === 'picked_up').length;
  ...
  return (
    <section className="grid grid-cols-4 gap-3 shrink-0">
      ...
      <div className="bg-white px-4 py-2 rounded-xl border border-slate-200 bg-slate-50 flex justify-between items-center shadow-xs">
        ...
        <span className="text-2xl font-black text-slate-800">{pickedUpCount}</span>
      </div>
    </section>
  );
}